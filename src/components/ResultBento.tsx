...
type ResultBentoProps = {
  ...
  dark: boolean;
  budgetTree: ReactNode;
};

export default function ResultBento({ options, dark, budgetTree }: ResultBentoProps) {
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        {options.map((opt, i) => (
            ...
            <div className="p-5 space-y-4">
                ...
                <span className={`font-medium ${dark ? "text-[#c2410c]" : "text-[#c2410c]"}`}>Fact: </span>
                {opt.funFact}
                </p>
            </div>
            </motion.article>
